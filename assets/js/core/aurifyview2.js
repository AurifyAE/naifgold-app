...
async function readSpreadValues() {
  try {
    const uid = "u5MUCo26GmQDVeFEZVFimwgd6vP2";
    if (!uid) {
...
async function readData() {
  ...
  const commoditiesCollectionRef = collection(
    firestore,
    `users/${uid}/commodities`
  );
  ...
  try {
    ...
    for (const data of tableData) {
      ...
      setInterval(async () => {
        ...
        if (
          metalInput === "Gold" ||
          metalInput === "Gold kilobar" ||
          metalInput === "Gold TOLA" ||
          metalInput === "Gold TEN TOLA"
        ) {
          newRow.innerHTML = `
                    <td style="text-align: right;" id="metalInput">Gold</td>
                    <td style="text-align: left; font-size:15px; font-weight: 600;">${purityInput}</td>
          ...
          if (weight === "GM" && unitInput < 1) {
            // Update the sellAED and buyAED values for the current
            newRow3.querySelector("#sellAED").innerText = parseFloat(
              (
                goldAskingPrice  *
                  unitInput *
                  unitMultiplier *
            ...
            newRow3.querySelector("#buyAED").innerText = (
             goldBiddingPrice  *
                unitInput *
                unitMultiplier *
                (purityInput / Math.pow(10, purityInput.length)) +
            ...
            newRow3.querySelector("#sellAED").innerText =
              parseInt(sellAEDValue).toFixed(0); // Round to remove decimals
            newRow3.querySelector("#buyAED").innerText =
              parseInt(buyAEDValue).toFixed(0); // Round to remove decimals
          }
